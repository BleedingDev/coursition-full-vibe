...
import { getCoursitionAuth } from '../server/coursition/auth.ts';
import { applyWorkflowActionWithCleanup, draftForOwner } from '../server/coursition/store.ts';
...
const sessionForRequest = (request: HttpServerRequest.HttpServerRequest) =>
  Effect.tryPromise({
    ...
    try: () =>
      getCoursitionAuth().then(({ auth }) =>
        auth.api.getSession({ headers: headersFromEffectRequest(request) }),
      ),
  });
    ...
    ownerIdForRequest(request).pipe(
      Effect.flatMap((ownerId) =>
        Effect.tryPromise({
            ...
            Schema.is(CoursitionWorkflowConflict)(cause)
              ? cause
              : new CoursitionServerError({ message: messageFrom(cause) }),
          try: () => applyWorkflowActionWithCleanup(ownerId, payload),
        }),
      ),
    ),
  ),
        ...
        Effect.flatMap((ownerId) =>
          Effect.gen(function* evaluateActivityProgram() {
            ...
            const evaluation = yield* Effect.tryPromise({
              ...
              try: () =>
                import('../server/coursition/ai-provider.ts').then(
