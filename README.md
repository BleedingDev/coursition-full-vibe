...
Coursition full AI mode uses an OpenAI-compatible server provider. Local
development defaults to the real local proxy at `http://localhost:8317/v1` with
the `gpt-5.3-codex-spark` model when no remote provider env is configured. The
proxy owns its OpenAI/Codex login; the app only sends OpenAI-compatible API
requests to it. Override it with `COURSITION_AI_BASE_URL`,
`COURSITION_AI_PROVIDER_API_KEY`, and `COURSITION_AI_MODEL`, or with
`OPENAI_BASE_URL` and `OPENAI_API_KEY`. Production mode always requires explicit
provider credentials and never uses the local deterministic fallback.
...
| Copy           | English and Czech resources in `locales`              |
...
`locales/<lang>/translation.json`, then render it through
